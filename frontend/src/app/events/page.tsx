  ...
  return (
    <div className="w-full max-w-7xl mx-auto px-4 py-12 md:py-24">
      ...
      <div className="mb-12 md:mb-16">
        <motion.h1 initial={{ opacity: 0, y: -20 }} animate={{ opacity: 1, y: 0 }} className="text-4xl sm:text-5xl md:text-7xl font-black mb-4 md:mb-6 tracking-tight text-[#111827]">
          Explore <span className="text-[#FACC15]">Events</span>
        </motion.h1>
        <motion.p initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ delay: 0.2 }} className="text-[#6B7280] text-xl max-w-2xl font-medium">
          Discover incredible experiences curated from around the world. Secure your spot at the most exclusive gatherings.
        </motion.p>
      </div>

      {/* Search & Filter Bar */}
      <div className="bg-white p-4 md:p-6 mb-12 md:mb-16 flex flex-col lg:flex-row gap-4 items-stretch lg:items-center rounded-2xl border border-gray-200 shadow-sm relative z-20">
        <div className="relative flex-1 w-full">
          ...
          <input 
            ...
            className="w-full bg-gray-50 border border-gray-300 rounded-xl py-3 md:py-4 pl-12 pr-4 text-[#111827] placeholder-[#9CA3AF] focus:outline-none focus:border-[#FACC15] focus:ring-2 focus:ring-yellow-50 transition-all font-medium text-sm md:text-base"
          />
        </div>
      ...
      {loading ? (
        <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-8">
          {[1, 2, 3, 4, 5, 6].map((n) => (
            <div key={n} className="h-[450px] bg-white border border-gray-200 rounded-2xl animate-pulse" />
          ))}
        </div>
      ) : events.length === 0 ? (
        <div className="bg-white p-16 text-center rounded-2xl border-2 border-dashed border-gray-300">
          ...
          <button onClick={() => {setSearch(''); setCategory('');}} className="mt-8 bg-white text-[#111827] font-bold py-3 px-6 rounded-xl border border-gray-300 hover:bg-gray-50 transition-all">Clear Filters</button>
        </div>
      ) : (
        <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-8">
          {events.map((event, index) => (
            <motion.div key={event.id} initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: index * 0.05 }}>
              <Link href={`/events/${event.id}`} className="block bg-white border border-gray-200 rounded-2xl overflow-hidden group h-[450px] flex flex-col hover:border-[#FACC15] hover:shadow-lg transition-all">
                <div className="h-56 bg-gray-200 relative overflow-hidden">
                  ...
                  <span className="absolute top-4 left-4 bg-[#FACC15] text-[#111827] px-3 py-1 rounded-full text-xs font-bold border border-[#EAB308]">
                    {event.category}
                  </span>
                </div>

                <div className="p-6 flex-1 flex flex-col relative bg-white">
                  <div className="bg-yellow-50 border border-yellow-200 rounded-xl p-3 inline-flex flex-col items-center justify-center w-16 shadow-sm mb-4 group-hover:border-[#FACC15] transition-colors z-10">
                  ...
                  <h3 className="text-xl font-black mb-2 text-[#111827] group-hover:text-[#FACC15] transition-colors line-clamp-1">{event.title}</h3>
                  <p className="text-[#6B7280] text-sm line-clamp-2 mb-4 font-medium flex-1">{event.description}</p>
