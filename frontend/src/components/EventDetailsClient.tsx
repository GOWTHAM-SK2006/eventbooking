...
export default function EventDetailsClient({ id: propsId }: EventDetailsClientProps) {
  ...
  if (loading) return <div className="flex justify-center items-center h-[70vh]"><Loader size={40} className="spin text-[#FACC15]" /></div>;
  ...
  return (
    <div className="w-full pb-24">
      ...
      {/* Hero Banner */}
      <div className="w-full h-[50vh] md:h-[65vh] relative bg-[#FAFAFA]">
        <img 
        ...
        <div className="absolute bottom-0 w-full px-4 pb-12 pt-32">
          <div className="max-w-7xl mx-auto flex flex-col md:flex-row gap-8 items-end justify-between">
            <div className="flex-1">
              <span className="inline-block px-4 py-1.5 bg-[#FACC15] text-[#111827] font-bold text-sm uppercase tracking-wider rounded-lg mb-4 shadow-[0_0_20px_rgba(255,107,0,0.4)]">
                {event.category}
              </span>
      ...
      <div className="max-w-7xl mx-auto px-4 mt-12 grid grid-cols-1 lg:grid-cols-3 gap-12">
        ...
        <div className="lg:col-span-2 space-y-12">
          {/* About Section */}
          <section>
            <h2 className="text-2xl font-black mb-6 flex items-center gap-3"><Sparkles className="text-[#FACC15]" /> Experience Overview</h2>
            <div className="glass-card p-8 md:p-10 text-[#6B7280] text-lg leading-relaxed whitespace-pre-line border-l-4 border-l-[#FACC15]">
              {event.description}
            </div>
          </section>
          ...
          <section className="grid grid-cols-1 md:grid-cols-2 gap-6">
            <div className="glass-card p-8 border border-[#E5E7EB]">
              <div className="w-12 h-12 bg-[#F9FAFB] rounded-2xl flex items-center justify-center mb-6 border border-[#D1D5DB]">
                <Calendar size={24} className="text-[#FACC15]" />
              </div>
              <h3 className="text-xl font-bold mb-2 text-[#111827]">Date and Time</h3>
              <p className="text-[#6B7280]">{new Date(event.startDate).toLocaleDateString('en-US', { weekday: 'long', year: 'numeric', month: 'long', day: 'numeric' })}</p>
              <p className="text-[#6B7280] font-medium mt-1">{new Date(event.startDate).toLocaleTimeString([], {hour: '2-digit', minute:'2-digit'})}</p>
            </div>
            ...
            <div className="glass-card p-8 border border-[#E5E7EB]">
              ...
              <p className="text-[#6B7280]">{event.location}</p>
              <p className="text-[#FACC15] text-sm font-semibold mt-3 cursor-pointer hover:underline">Get Directions →</p>
            </div>
          </section>
          ...
          <section>
            ...
            <div className="glass-card p-8 space-y-8">
              {session ? (
                <form onSubmit={handleReviewSubmit} className="bg-[#FFFFFF] p-6 rounded-2xl border border-[#E5E7EB]">
                  <h4 className="font-bold mb-4 text-[#111827]">Share your experience</h4>
                  <div className="flex gap-2 mb-4">
                    {[1, 2, 3, 4, 5].map((star) => (
                      <button type="button" key={star} onClick={() => setReviewRating(star)} className="focus:outline-none transition-transform hover:scale-110">
                        <Star size={24} fill={star <= reviewRating ? '#FACC15' : 'none'} color={star <= reviewRating ? '#FACC15' : '#333'} />
                      </button>
                    ))}
                  </div>
                  <textarea 
                    value={reviewComment} onChange={(e) => setReviewComment(e.target.value)} required
                    className="w-full bg-[#FFFFFF] border border-[#D1D5DB] rounded-xl p-4 text-[#111827] focus:outline-none focus:border-[#FACC15] focus:ring-1 focus:ring-[#FACC15] transition-colors mb-4 resize-none"
                ...
                <div className="bg-[#FFFFFF] p-6 rounded-2xl border border-[#E5E7EB] text-center">
                  <p className="text-[#6B7280] mb-4">You must be logged in to leave a review.</p>
              ...
              <div className="space-y-6 pt-6">
                {reviews.length === 0 ? (
                  <p className="text-center text-[#6B7280] py-8 italic">No reviews yet. Be the first!</p>
                ) : (
                  reviews.map((r: any, idx) => (
                    <div key={idx} className="border-b border-[#E5E7EB] pb-6 last:border-0">
                      <div className="flex justify-between items-start mb-2">
                        <div className="flex items-center gap-3">
                          <div className="w-10 h-10 bg-gradient-to-br from-[#FACC15] to-[#FF8C42] rounded-full flex items-center justify-center text-[#111827] font-bold shadow-lg">
                            {r.userName?.charAt(0) || 'U'}
                          </div>
                          <div>
                            <p className="font-bold text-[#111827]">{r.userName || 'Anonymous'}</p>
                            <p className="text-xs text-[#9CA3AF]">{new Date(r.createdAt).toLocaleDateString()}</p>
                          </div>
                        </div>
                        <div className="flex gap-1">
                          {[...Array(5)].map((_, i) => (
                            <Star key={i} size={14} fill={i < r.rating ? '#FACC15' : 'none'} color={i < r.rating ? '#FACC15' : '#333'} />
                          ))}
                        </div>
                      </div>
                      <p className="text-[#6B7280] mt-3 pl-13 leading-relaxed">{r.comment}</p>
                    </div>
                  ))
                )}
              </div>
        ...
        <div className="lg:col-span-1" id="booking-section">
          <div className="sticky top-28 glass-card border-t-4 border-t-[#FACC15] shadow-2xl">
            <div className="p-8">
              <div className="mb-8">
                <span className="text-[#6B7280] text-sm uppercase tracking-wider font-bold">Access Pass</span>
                <div className="text-4xl font-black text-[#111827] mt-1">
              ...
              {event.availableSlots <= 0 ? (
                ...
                <form onSubmit={handleBookingSubmit} className="space-y-6">
                  <div>
                    <label className="block text-sm font-bold text-[#111827] mb-2">Select Quantity</label>
                    <select 
                      value={quantity} onChange={(e) => setQuantity(Number(e.target.value))}
                      className="w-full bg-[#F9FAFB] border border-[#D1D5DB] rounded-xl p-4 text-[#111827] font-bold focus:outline-none focus:border-[#FACC15] appearance-none cursor-pointer"
                    >
                  ...
                  <div className="bg-[#FFFFFF] p-4 rounded-xl border border-[#E5E7EB]">
                    ...
                    <div className="flex justify-between items-center pt-3 border-t border-[#E5E7EB]">
                      ...
                      <span className="font-black text-xl text-[#FACC15]">₹{totalPrice.toLocaleString('en-IN', {minimumFractionDigits:2})}</span>
                    </div>
                  </div>
                  ...
                  <p className="text-center text-xs text-[#9CA3AF] mt-4 flex justify-center items-center gap-1">
      ...
      <AnimatePresence>
        {showPaymentModal && (
          <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="fixed inset-0 z-[100] flex items-center justify-center p-4 bg-black/80 backdrop-blur-sm">
            <motion.div initial={{ scale: 0.95, y: 20 }} animate={{ scale: 1, y: 0 }} exit={{ scale: 0.95, y: 20 }} className="bg-[#FFFFFF] border border-[#E5E7EB] rounded-3xl w-full max-w-md overflow-hidden shadow-[0_20px_50px_rgba(0,0,0,0.5)]">
              
              {bookingSuccess ? (
                <div className="p-6 sm:p-10 text-center flex flex-col items-center">
                  <motion.div initial={{ scale: 0 }} animate={{ scale: 1 }} transition={{ type: "spring" }} className="w-24 h-24 bg-[#FACC15]/20 rounded-full flex items-center justify-center mb-6">
                ...
                <div className="flex flex-col h-full">
                  <div className="px-6 py-4 border-b border-[#E5E7EB] flex justify-between items-center bg-[#FFFFFF]">
                    <h3 className="text-xl font-bold text-[#111827]">Checkout</h3>
                    <button onClick={() => setShowPaymentModal(false)} className="text-[#6B7280] hover:text-[#111827] transition-colors bg-[#F9FAFB] p-2 rounded-full"><X size={20} /></button>
                  </div>
                  
                  <div className="p-6">
                    <div className="bg-[#FAFAFA] p-4 sm:p-6 rounded-2xl border border-[#E5E7EB] mb-6 sm:mb-8 text-center">
                      <p className="text-[#6B7280] text-xs sm:text-sm font-bold uppercase tracking-wider mb-2">Total Amount Due</p>
                      <h4 className="text-3xl sm:text-4xl font-black text-[#FACC15]">₹{totalPrice.toLocaleString('en-IN', {minimumFractionDigits:2})}</h4>
                    </div>

                    {event.price > 0 && (
                      <div className="space-y-3 mb-8">
                        <p className="text-sm font-bold text-[#111827] mb-4">Select Payment Method</p>
                        
                        <label className={`flex items-center p-4 border rounded-xl cursor-pointer transition-all ${paymentMethod === 'CARD' ? 'border-[#FACC15] bg-[#FACC15]/5' : 'border-[#D1D5DB] bg-[#F9FAFB] hover:border-[#444]'}`}>
                          <input type="radio" value="CARD" checked={paymentMethod === 'CARD'} onChange={() => setPaymentMethod('CARD')} className="hidden" />
                          <div className={`w-5 h-5 rounded-full border flex items-center justify-center mr-4 ${paymentMethod === 'CARD' ? 'border-[#FACC15]' : 'border-[#9CA3AF]'}`}>
                            {paymentMethod === 'CARD' && <div className="w-2.5 h-2.5 bg-[#FACC15] rounded-full" />}
                          </div>
                          <CreditCard className="mr-3 text-[#6B7280]" size={20} />
                          <span className="font-bold text-[#111827]">Cards, NetBanking, Wallets</span>
                        </label>

                        {paymentSettings?.upiId && (
                          <label className={`flex items-center p-4 border rounded-xl cursor-pointer transition-all ${paymentMethod === 'UPI' ? 'border-[#FACC15] bg-[#FACC15]/5' : 'border-[#D1D5DB] bg-[#F9FAFB] hover:border-[#444]'}`}>
                            <input type="radio" value="UPI" checked={paymentMethod === 'UPI'} onChange={() => setPaymentMethod('UPI')} className="hidden" />
                            <div className={`w-5 h-5 rounded-full border flex items-center justify-center mr-4 ${paymentMethod === 'UPI' ? 'border-[#FACC15]' : 'border-[#9CA3AF]'}`}>
                              {paymentMethod === 'UPI' && <div className="w-2.5 h-2.5 bg-[#FACC15] rounded-full" />}
                            </div>
                            <QrCode className="mr-3 text-[#6B7280]" size={20} />
                            <span className="font-bold text-[#111827]">Manual UPI / QR Code</span>
                          </label>
                        )}
                      </div>
                    )}
                    ...
                    {paymentMethod === 'UPI' && event.price > 0 ? (
                      <div className="bg-[#FFFFFF] p-6 rounded-2xl border border-[#E5E7EB] space-y-4 mb-8">
                        <div className="flex justify-center mb-4">
                          {paymentSettings?.qrCodeUrl ? (
                            <img src={paymentSettings.qrCodeUrl} alt="UPI QR" className="w-48 h-48 rounded-xl border border-[#333]" />
                          ) : (
                            <div className="w-48 h-48 bg-[#F9FAFB] rounded-xl flex items-center justify-center text-[#9CA3AF] border border-[#333]">No QR Set</div>
                          )}
                        </div>
                        <p className="text-center font-mono font-bold text-[#111827] bg-[#F9FAFB] py-2 rounded-lg">{paymentSettings?.upiId}</p>
                        <input 
                          ...
                          className="w-full bg-[#F9FAFB] border border-[#D1D5DB] text-[#111827] rounded-xl px-4 py-3 focus:outline-none focus:border-[#FACC15] font-medium"
                        />
                      </div>
                    ) : null}
      ...
      {!loading && event && (
        <div className="lg:hidden fixed bottom-16 left-0 right-0 bg-[#FFFFFF] border-t border-[#E5E7EB] p-4 z-40 flex justify-between items-center shadow-[0_-10px_30px_rgba(0,0,0,0.5)]">
          <div>
            <p className="text-[#6B7280] text-xs font-bold uppercase">Price</p>
            <p className="text-xl font-black text-[#111827]">{event.price === 0 ? 'Free' : `₹${event.price.toLocaleString('en-IN')}`}</p>
          </div>
          <button 
            onClick={() => {
